...
const composeLocalTemplate = `services:
  ...
  dashboard:
    image: ${DASHBOARD_IMAGE:-manga-cdc-dashboard:local}
    build:
      context: ./dashboard
      dockerfile: Dockerfile
    restart: unless-stopped
...
func renderComposeLocal(m manifest.Manifest) (string, error) {
	if m.Tier != manifest.TierLocal {
		return "", fmt.Errorf("compose local: tier must be %q", manifest.TierLocal)
	}
	return composeLocalTemplate, nil
}

func writeComposeLocal(m manifest.Manifest) error {
	content, err := renderComposeLocal(m)
	if err != nil {
		return err
	}
	return writeGeneratedFile("docker-compose.yml", content)
}
