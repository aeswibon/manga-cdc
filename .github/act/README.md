...
| Push tag `v*` | **Release** → `version-sync` → test/tf/build stages → `github-release` → `deploy-gcp` → `deploy-vercel` |
...
`version-sync` commits version file bumps to `master` but **does not move the release tag** mid-pipeline. `release-gate.sh` skips a second run when the tagged commit contains `[skip release]`, or when a tag **update** points at `master` and release images already exist.
